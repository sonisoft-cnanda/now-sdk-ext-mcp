...
describe('lookup_app tool', () => {
  ...
  describe('combined search (type=all)', () => {
    it('should query sys_scope and sys_plugins and return combined results', async () => {
            ...
            result: [
              {
                sys_id: 'app-123',
                name: 'Acme SDK',
                scope: 'x_acme_acme_sdk',
                version: '2.1.0',
                vendor: 'Acme Corp',
                active: 'true',
                short_description: 'Acme integration for ServiceNow',
      ...
      expect(text).toContain('x_acme_acme_sdk')
    ...
    it('should pass the instance alias to withConnectionRetry', async () => {
      ...
      await client.callTool({
        name: 'lookup_app',
        arguments: { search_term: 'test', instance: 'myinstance' },
      })
  ...
  describe('query construction', () => {
    it('should use LIKE matching on name, scope, and short_description for sys_scope', async () => {
      ...
      expect(q).toContain('nameLIKEacme')
      expect(q).toContain('scopeLIKEacme')
      expect(q).toContain('short_descriptionLIKEacme')
